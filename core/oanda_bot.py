...
logging.basicConfig(
    filename='oanda_bot.log',
    format='%(asctime)s %(levelname)s: %(message)s',
    level=logging.INFO
)
logger = logging.getLogger(__name__)
logger.info("Démarrage du script oanda_bot.py")

# Forcer l’importation depuis utils/
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from utils.data_fetch import fetch_oanda_prices
from utils.kalman_filter import apply_kalman_filter
from utils.stat_arb import stat_arb_opportunity

async def oanda_roadmap():
    """Génère la roadmap pour Oanda."""
    try:
        now = datetime.now()
        if now.weekday() >= 5:
            logger.info("Week-end : marchés Oanda fermés, aucune roadmap générée")
            with open('roadmaps/oanda_roadmap.json', 'w') as f:
                json.dump([], f, indent=2)
            return []

        logger.info("Début de la génération de la roadmap Oanda")
        ...
        # Récupérer les rendements des actifs Oanda
        symbols = ['EUR_USD', 'GBP_USD', 'USD_JPY', 'XAU_USD', 'BCO_USD', 'NATGAS_USD']
        returns = {}
        for symbol in symbols:
            try:
                returns[symbol] = fetch_oanda_prices(symbol)
                logger.info(f"Rendements récupérés pour {symbol} : {returns[symbol]}")
            except requests.exceptions.RequestException as e:
                logger.error(f"Erreur réseau récupération rendements pour {symbol} : {str(e)}")
                returns[symbol] = [0]
        ...
        logger.info(f"Roadmap Oanda sauvegardée : {roadmap}")
        return roadmap
    except Exception as e:
        logger.error(f"Erreur oanda_roadmap : {str(e)}")
        return []

async def main():
    """Boucle principale du bot Oanda."""
    logger.info("Démarrage boucle principale")
    try:
        await oanda_roadmap()
    except Exception as e:
        logger.error(f"Erreur test immédiat oanda_roadmap : {str(e)}")
    while True:
        try:
            now = datetime.now()
            ...
            if now.weekday() < 5 and now.hour == pre_market_time.hour:
                await oanda_roadmap()
...
if __name__ == '__main__':
    logger.info("Lancement du bot Oanda")
    asyncio.run(main())
