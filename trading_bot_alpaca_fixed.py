...
class AlpacaTrader:
    ...
    def execute_iron_condor(self, strategy):
        ...
        try:
            # Vendre call spread
            self._place_order(
                symbol=f"{symbol}{str(int(strategy['call_short_strike']))}C",
                qty=qty,
                side=OrderSide.SELL
            )
            self._place_order(
                symbol=f"{symbol}{str(int(strategy['call_long_strike']))}C",
                qty=qty,
                side=OrderSide.BUY
            )
            ...
            # Vendre put spread
            self._place_order(
                symbol=f"{symbol}{str(int(strategy['put_short_strike']))}P",
                qty=qty,
                side=OrderSide.SELL
            )
            self._place_order(
                symbol=f"{symbol}{str(int(strategy['put_long_strike']))}P",
                qty=qty,
                side=OrderSide.BUY
            )
...
if __name__ == "__main__":
    print("="*60)
    print("SYSTÈME DE TRADING AUTONOME AVEC ALPACA (CORRIGÉ)")
    print("="*60)
