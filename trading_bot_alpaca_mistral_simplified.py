# trading_bot_alpaca_mistral_simplified.py
...
class AlpacaTrader:
    ...
    def execute_iron_condor(self, strategy):
        ...
        call_short = 5 * round(float(strategy['call_short_strike']) / 5)
        call_long = 5 * round(float(strategy['call_long_strike']) / 5)
        put_short = 5 * round(float(strategy['put_short_strike']) / 5)
        put_long = 5 * round(float(strategy['put_long_strike']) / 5)
        ...
        try:
            ...
            call_short_symbol = convert_to_alpaca_option_symbol(symbol, call_short, "call", expiration)
            call_long_symbol = convert_to_alpaca_option_symbol(symbol, call_long, "call", expiration)
            put_short_symbol = convert_to_alpaca_option_symbol(symbol, put_short, "put", expiration)
            put_long_symbol = convert_to_alpaca_option_symbol(symbol, put_long, "put", expiration)
    ...
    def _place_order(self, symbol, qty, side):
        ...
        try:
            ...
            # Essayez avec le format de symbole alternatif
            if "not found" in str(e):
                print("⚠️ Tentative avec format de symbole alternatif...")
                ...
                try:
                    ...
                    print(f"✅ Ordre {side} exécuté avec format alternatif: {qty} {alt_symbol}")
                    return order
                except:
                    pass
            raise
...
if __name__ == "__main__":
    print("="*60)
    print("SYSTÈME DE TRADING AUTONOME ALPACA-MISTRAL (SIMPLIFIÉ)")
    print("="*60)
